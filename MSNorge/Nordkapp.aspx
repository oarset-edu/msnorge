<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Nordkapp.aspx.cs" Inherits="MSNorge.Nordkapp" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
    <link rel="stylesheet" type="text/css" href="main.css" id="main_css">
</head>
<body>
    <form id="MSform" runat="server">
    <div id="content">
        <div id="header">

        </div>
        <div id ="main">
            <div id ="background">
                <img src="img/Fra_motgang_til_Nordkapp_side.jpg" id="undersidebilde"/>
            </div>
            <div id="menu">
                <a href="https://instagram.com/ms_norge/">
                    <img src="img/insta_icon2.png" id="insta"class="logo"/>
                </a>
                <a href="https://www.facebook.com/framotgangtilnordkapp" id="fb">
                    <img src="img/fb_icon2_circle.png" id="fb"class="logo"/>
                </a>
                <a href="mailto:steinar@msnorge.no" >
                    <img src="img/contact_icon2.png" id="contact"class="logo"/>
                </a>
            </div>
        </div>
        <div id ="undersidetekst">
            <h1>
                KOMMER
            </h1>
        </div>

    </div>
    </form>
</body>
</html>
